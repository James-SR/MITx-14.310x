...
Two of the more common Kernels are the Epanechnikov and Normal distribution, with the former being bounded (see second link above). If the bandwidth is too wide, we will not fit the data well and introduce bias.  If the bandwidth is too narrow, we overfit our data.  The goal is to be somewhere in the middle and to minimise the MSE.
...
Bihar6 <- ggplot(bihar_adult_females_trunc, aes(height_cm)) + 
  geom_histogram(data = bihar_adult_females_trunc, aes(height_cm, ..density..), fill = "white", color = "darkred") +
  stat_density(kernel = "gaussian", bw = 5, aes(height_cm), fill = NA, color = "darkblue") +
  xlab("bandwidth = 5")
