...
histdata_twoyears <- teenager_fr %>%
  dplyr::slice(46:263) %>%
  dplyr::select(Country.Code, X1960, X2000) %>%
  gather("Year", "FertilityRate", 2:3)

ggplot(histdata_twoyears, aes(x = FertilityRate)) + 
  geom_histogram(data = histdata_twoyears, Year=="X1960", colour = "darkred", fill = "red", alpha = 0.2) +
  geom_histogram(data = histdata_twoyears, Year=="X2000", colour = "darkblue", fill = "blue", alpha = 0.2) 
```
